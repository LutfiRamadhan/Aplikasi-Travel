        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title-wrap bar-success">
                    <h4 class="card-title">Edit Data Jadwal {{ date('d F Y', strtotime($jadwal->tanggal)) }}</h4>
                </div>
            ...
            <div class="card-body">
                <div class="card-block">
                    ...
                    @endif
                    <form action="{{ url('jadwal/'.$jadwal->id) }}" method="post" class="form-horizontal">
                        <div class="form-group row">
                            <label for="tanggal" class="label-control col-md-3">
                                Tanggal :
                            </label>
                            <div class="col-md-9">
                                <input type="date" id="tanggal" name="tanggal" class="form-control" required placeholder="Masukan Tanggal" data-validation-required-message="Tanggal harus diisi" value="{{ date('Y-m-d', strtotime($jadwal->tanggal)) }}" min="{{ date('Y-m-d') }}" max="{{ date('Y-m-d', strtotime('+7 day')) }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="time" id="jam_berangkat" name="jam_berangkat" class="form-control" required placeholder="Masukan Jam berangkat" data-validation-required-message="Jam berangkat harus diisi" value="{{ $jadwal->jam_berangkat }}" min="00:00" max="23:59">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="time" name="perkiraan_tiba" id="perkiraan_tiba" placeholder="Masukan Perkiraan Tiba" class="form-control" required  data-validation-required-message="Perkiraan Tiba harus diisi" value="{{ $jadwal->perkiraan_tiba }}">
                            ...
                            <div class="col-md-9">
                                <select name="id_rute" id="rute" class="form-control" required data-validation-required-message="Rute harus dipilih">
                                    ...
                                    @foreach ($rute as $item)
                                        <option value="{{ $item->id }}" {{ ($item->id == $jadwal->id_rute)?'selected':'' }}>{{ $item->kota_asal . ' - ' . $item->kota_destinasi }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-9">
                                <select name="id_kendaraan"  id="kendaraan" class="form-control" required data-validation-required-message="Kendaraan harus dipilih">
                                    ...
                                    @foreach ($kendaraan as $item)
                                        <option value="{{ $item->id }}" {{ ($item->id == $jadwal->id_kendaraan)?'selected':'' }}>{{ $item->merk . ' - ' . $item->tipe_jenis }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->first('kendaraan') != "")
                                    <div class="help-block">
                                        <ul role="alert">
                                            <li>{{ $errors->first('kendaraan') }}</li>
                                        </ul>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="col-md-9">
                                <select name="id_supir" id="supir" class="form-control" required data-validation-required-message="Supir harus dipilih">
                                    ...
                                    @foreach ($supir as $item)
                                        <option value="{{ $item->id }}" {{ ($item->id == $jadwal->id_supir)?'selected':'' }}>{{ $item->nama }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->first('supir') != "")
                                    <div class="help-block">
                                        <ul role="alert">
                                            <li>{{ $errors->first('supir') }}</li>
                                        </ul>
                                    </div>
                                @endif
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input type="number" id="harga" name="harga" class="form-control" required placeholder="Masukan Harga" data-validation-required-message="Harga harus diisi" min="50000" max="1000000" value="{{ $jadwal->harga }}">
